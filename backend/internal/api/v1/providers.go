...
// Body: { provider_name, base_url, api_key, model }
func (h *ProviderHandler) testConnection(w http.ResponseWriter, r *http.Request) {
	...
	result := h.svc.TestConnection(r.Context(), req.ProviderName, req.BaseURL, req.APIKey, req.Model)
	jsonOK(w, result)
}
...
func (h *ProviderHandler) createProfile(w http.ResponseWriter, r *http.Request) {
	...
	p, err := h.svc.Create(r.Context(),
		req.Name, req.ProviderName, req.BaseURL, req.APIKeyEnv, req.APIKey,
...
func (h *ProviderHandler) testProfile(w http.ResponseWriter, r *http.Request) {
	result := h.svc.TestProfile(r.Context(), chi.URLParam(r, "id"))
	jsonOK(w, result)
}
